...
app.get('/api/resolve', async (req, res) => {
  ...
  try {
    const r = await axios.get(url, { timeout: 5000, responseType: 'text' });
...
app.get('/stream-proxy', async (req, res) => {
  ...
  try {
    // If target points to host's mapped port, rewrite to internal docker service
    let internal = target;
    try {
      const u = new URL(target);
      if ((u.hostname === 'localhost' || u.hostname === '127.0.0.1') && (u.port === '8099' || u.port === '8000')) {
        u.hostname = 'icecast';
        u.port = '8000';
        u.protocol = 'http:';
        internal = u.toString();
